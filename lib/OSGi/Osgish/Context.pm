...
OSGi::Osgish::Context - Context for osgish commands

=head1 DESCRIPTION

This context is pushed to commands and allows access to all relevant
informations shared between commands. A command should consult the 
context when performing its operation for contacting the OSGi server. 
The context gets updated in the background e.g. when the server changes. 
...
=item $context = new OSGi::Osgish::Context(osgish => $osgish,...)

Constructor, which is used internally by C<osgish> to create the context
which gets pushed to the commands.
...
=item $osgish = $context->osgish

Access to the osgish object for accessing to the connected server. If there 
is no connected server, this methods returns C<undef>

=cut 

sub osgish {
    return shift->_get_set("osgish",@_);
}
